...
export default function ExpensesPage() {
  ...
  useEffect(() => {
    if (user) {
      expensesApi.list(filter ? { status: filter } : {})
        .then((data: any) => setExpenses(data.expenses || []))
        .catch(console.error)
      ...
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        {fetching ? (
          ...
          <div className="card text-center py-12">
            ...
            <p className="text-gray-500">No hay solicitudes{filter ? ` con estado "${statusLabel[filter]?.label}"` : ''}</p>
